---
layout: post
title: Xử lý số nguyên lớn (Bigint)🤠
...
int test() {
    ...
    for (int i = 1; i < TEST_CASES + 1; i++) {
        ...
        bigint a = bigint(first);
        bigint b = bigint(second);
        bigint ans = a + b;

        long long result = stoll(first) + stoll(second);
        if (ans != result) {
    ...
    for (int i = 1; i < TEST_CASES + 1; i++) {
        ...
        bigint a(first);
        bigint b_shift(shift_val);
        bigint ans = a << b_shift;

        long long result = stoll(first) << shift_val;
        if (ans != result) {
            cout << "failed at case #" << i << ". (" << a << " << " << shift_val << " = " << ans << "), expected: " << result << '\n';
            exit(0);
        }
    }
    cout << "passed!" << '\n';
    ...
    for (int i = 1; i < TEST_CASES + 1; i++) {
        ...
        long long result = stoll(first) >> stoll(shift_val);
        if (ans != result) {
            cout << "failed at case #" << i << ". (" << a << " >> " << shift_val << " = " << ans << "), expected: " << result << '\n';
            exit(0);
        }
